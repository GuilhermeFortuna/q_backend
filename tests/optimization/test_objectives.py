...
def test_maximize_net_profit():
    assert resolve_objective({"total_pnl": 500.0}, ObjectiveMode.MAXIMIZE_NET_PROFIT) == 500.0


def test_maximize_sharpe():
    assert resolve_objective({"sharpe_ratio": 1.5}, ObjectiveMode.MAXIMIZE_SHARPE) == 1.5


def test_minimize_drawdown():
    assert resolve_objective({"max_drawdown_pct": 0.12}, ObjectiveMode.MINIMIZE_DRAWDOWN) == 0.12


def test_maximize_return_drawdown():
    assert resolve_objective({"return_drawdown_ratio": 3.0}, ObjectiveMode.MAXIMIZE_RETURN_DRAWDOWN) == 3.0
