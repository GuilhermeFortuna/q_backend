...
class BacktestEngine:
    ...
    def _run_single_chunk(self, chunk: pd.DataFrame, force_close_at_end: bool) -> TradeRegistry:
        ...
        for i in range(len(chunk)):
            ...
            exit_signals = self.strategy.check_exit_conditions(current_data, registry.get_open_trades())
            for sig in exit_signals:
                if sig.action == SignalAction.CLOSE:
                    ...
                    for t in open_trades:
                        closed_trade = registry.close_trade(t.id, timestamp, current_price)
