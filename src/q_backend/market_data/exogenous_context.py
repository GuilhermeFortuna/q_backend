...
def get_cached_exogenous_provenance(
    *,
    ...
    key = _evaluation_cache_key(
        primary_symbol, primary_timeframe, start, end, exogenous_series
    )
...
def resample_completed_bars(
    ...
    *,
    ...
    if bar_duration(src) >= bar_duration(tgt):
        raise ValueError(
            f"Cannot resample {source_timeframe} to finer timeframe {target_timeframe}"
        )
...
def _preflight_overlap(
    ...
    *,
    ...
    if source_end < primary_start:
        raise ValueError(
            f"Exogenous series {symbol} does not overlap the primary evaluation range"
        )
    ...
    if valid.empty:
        raise ValueError(
            f"Exogenous alignment for {symbol} produced no overlapping observations"
        )
...
def align_exogenous_close(
    ...
    *,
    ...
    avail_times = (
        _bar_close_times(exog.index, aligned_timeframe)
        + availability_lag_bars * bar_duration(primary_timeframe)
    )
    right = pd.DataFrame(
        {
...
def _compute_recipe_columns(
    *,
    ...
    if "return_zscore" in spec.recipes:
        columns[exog_column_name(symbol, f"return_zscore_{lookback}_{spec.vol_window}")] = (
            compute_rolling_zscore(exog_return, spec.vol_window)
        )

    if "rolling_corr" in spec.recipes:
        columns[exog_column_name(symbol, f"rolling_corr_{spec.corr_window}")] = (
            primary_return.rolling(spec.corr_window).corr(exog_return)
        )

    if "relative_strength" in spec.recipes:
        columns[exog_column_name(symbol, f"relative_strength_{lookback}")] = (
            primary_return - exog_return
        )
    ...
    if "direction_regime" in spec.recipes:
        columns[exog_column_name(symbol, f"direction_regime_{lookback}")] = (
            exog_return > 0
        ).astype(float)
...
def attach_exogenous_context(
    ...
    *,
    ...
    for spec in exogenous_series:
        ...
        _preflight_source_continuity(
            source, symbol=spec.symbol, timeframe=spec.source_timeframe
        )
        ...
        provenance.append(
            {
                ...
                "target_timeframe": (
                    spec.target_timeframe.upper() if spec.target_timeframe else None
                ),
                "availability_lag_bars": spec.availability_lag_bars,
